...
if (strlen($search_string) >= 1 && $search_string !== ' ') {
	// Build Query
	$query = 'SELECT * FROM jadwal WHERE kd_matkul LIKE "%'.$search_string.'%" OR name LIKE "%'.$search_string.'%"';
	...
	if (isset($result_array)) {
		...
		$output = str_replace('nameString', '<b>Hasil tidak ditemukan.</b>', $output);
		$output = str_replace('functionString', 'Silahkan masukan kata dengan benar :)', $output);
